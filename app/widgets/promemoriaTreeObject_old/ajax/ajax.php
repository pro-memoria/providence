...
switch ( $operation ) {
    ...
    case "get_children_contestuale":
        ...
        while ( $qr_result->nextRow() ) {
            ...
            if ($numerazione == "") {
                $attr_num_puntamento = $o_db->query("SELECT v.value_longtext1 as 'value' FROM ca_attribute_values v INNER JOIN ca_attributes a ON (a.attribute_id = v.attribute_id) WHERE v.element_id = 197 AND a.table_num = 57 AND a.row_id = {$objectId}");
                if ($attr_num_puntamento->nextRow())  {
                    $numerazione = $attr_num_puntamento->get('value');
                }
            }
            $text = $tipologia . $nome . " " . $numerazione;
