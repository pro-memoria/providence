...
function saveChildren( $id, $parent_id = "null", $pos ) {
    ...
    //foreach ( $children as $posizione => $id ) {
        ( $parent_id == 'null' ) ? $hier_occurrence_id = $id : $hier_occurrence_id = $parent_id;
        $o_db->query( "UPDATE ca_occurrences SET parent_id = $parent_id, hier_occurrence_id = $hier_object_id  WHERE occurrence_id = $id" );
    //}
}
...
switch ( $operation ) {
    case "get_children":
        ...
        if ($user_id == 1) {
            ...
            $query = "
                SELECT t.occurrence_id as id, t.parent_id,t.type_id as type, l.name as text
                FROM
                ca_occurrences t
                inner join
                ca_occurrence_labels l on (t.occurrence_id=l.occurrence_id)
				WHERE deleted=0 and ";
        } else {
            $query = "SELECT t.occurrence_id as id, t.parent_id,t.type_id as type, l.name as text
                      FROM
                      ((ca_occurrences t inner join ca_occurrence_labels l on (t.occurrence_id=l.occurrence_id) INNER JOIN ca_acl ON t.occurrence_id = ca_acl.row_id AND ca_acl.table_num = 67))
