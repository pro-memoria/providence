...
function getChildren( $id ) {
	...
	$query    = 'SELECT ca_objects.object_id as "id", ca_object_labels.name as "name", ca_objects.type_id as "type", ca_objects.access as "access"
			...
			WHERE ca_objects.parent_id = ' . $id . ' AND ca_object_labels.is_preferred = 1 AND ca_objects.deleted = 0 ORDER BY ca_objects.ordine';
	...
	while ($result->nextRow() ) {
		$count   = "SELECT COUNT(*) as 'num'
						FROM ca_objects
						WHERE ca_objects.parent_id = {$result->get('id')} AND ca_objects.deleted = 0";
		$r       = $mysql->query( $count );
		$r->nextRow();
		$isChild = ( $r->get('num') > 0 ) ? false : true;
