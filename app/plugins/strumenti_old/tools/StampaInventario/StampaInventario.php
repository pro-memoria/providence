...
class StampaInventario {
    ...
    public function run($start_id, $typeXview = array()) {
        ...
        while (!$this->stack->isEmpty()) {
            ...
            if ($object->getTypeID() == $types['Fondi'])   {
                ...
                $header = $currentSection->createHeader();
                $header->addText($this->getPreferredLabel($object));
                $this->printTitle($currentSection, $this->getPreferredLabel($object), 'h1');
            } else {
                if ($type['isad'] == 'ISAD3' || $type['isad'] == 'ISAD4')   {
                    if ($type['isad'] == 'ISAD3')   {
                        $this->printTitle($currentSection, $this->getPreferredLabel($object), 'h5', $currentLivel, true);
                    } else {
                        $this->printTitle($currentSection, $this->getPreferredLabel($object), 'h5', $currentLivel, false);
                    }
                } else {
                    $h = 'h' . (filter_var($type['isad'], FILTER_SANITIZE_NUMBER_INT) + $currentLivel);
                    $this->printTitle($currentSection, $this->getPreferredLabel($object), $h, $currentLivel);
                }
                // $paragraph = $this->newParagraph($currentSection, 2);
            }

            $metadataView = $this->typeXview[$object->getTypeID()];
            if ($metadataView != null)  {
                ...
                foreach($va_placements as $id => $placement)   {
                    $value = $t_display->getDisplayValue($object, $id, array('convertCodesToDisplayText' => true));
                    if ($value) {
                        if ($placement['bundle_name'] == 'ca_objects.type_id') {
                            $paragraph->addText(self::encodeText("Tipologia:"), array("bold" => true));
                            $paragraph->addText(self::encodeText(strip_tags($value)) . "\n", array("bold" => false));
                        } else {
                            ...
                            if ($boldContain->length > 0) {
                                foreach ($boldContain as $bld) {
                                    $paragraph->addText(self::encodeText(strip_tags($bld->nodeValue)), array("bold" => true));
                                    $paragraph->addText(self::encodeText(strip_tags($bld->nextSibling->nodeValue)), array("bold" => false));
                                }
                            } else {
                                $content = self::br2nl($value);
                                $textlines = explode("\n", $content);
                                foreach ($textlines as $lines) {
                                    $paragraph->addText(self::encodeText(strip_tags($lines)) . "\n");
    ...
    private function frontpage() {
        $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor(__CA_BASE_DIR__ . '/import/TemplateInventario.docx');
    ...
    private function printTitle(&$section, $title, $h = 'h3', $livel = 0, $bold = true) {
        $hn = $this->config->get('heading');
        $fontStyle = array('bold' => $bold, 'size' => $hn[$h]);
        $paragraph = array('spaceAfter' => \PhpOffice\PhpWord\Shared\Converter::cmToTwip(0), 'spaceBefore' => \PhpOffice\PhpWord\Shared\Converter::cmToTwip(0));
        $pa = $this->newParagraph($section, $livel);
        $pa->addText(self::encodeText($title), $fontStyle, $paragraph);
    }
    ...
    private function newParagraph(&$section, $indentation = 0, $brakePage = false, $space = 0.4) {
        $style = array(
            'indent' => $indentation,
